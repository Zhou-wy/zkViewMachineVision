...
namespace YOLOv8Seg
{
    ...
    public:
        ...
        void worker(std::promise<bool> &result) override
        {
             ...
             * 8400 -> 3 anchar
             */
            auto bbox_head_dims_ = engine->tensor("output0");
            ...
            int num_classes = bbox_head_dims_->size(2) - 4 - segment_head_dims_->size(1);
            ...
            while (get_jobs_and_wait(fetch_jobs, max_batch_size))
            {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
                {
                    ...
                    decode_kernel_invoker(image_based_output, bbox_head_dims_->size(1), num_classes, bbox_head_dims_->size(2), confidence_threshold_,
                                          nms_threshold_, affine_matrix, output_array_ptr, MAX_IMAGE_BBOX, NUM_BOX_ELEMENT, Task::seg, stream_);
                }
                            ...
                            if (task_ == Task::seg)
                            {
                                ...
                                if (mask_out_width > 0 && mask_out_height > 0)
                                {
                                    ...
                                    float *mask_weights = bbox_head_dims_->gpu<float>(ibatch) +
                                                          (ibatch * bbox_head_dims_->size(1) + row_index) * bbox_head_dims_->size(2) +
                                                          num_classes + 4;

                                    unsigned char *mask_out_device = output_segment_device.gpu<unsigned char>(ibatch);

                                    decode_single_mask(left * scale_to_predict_x, top * scale_to_predict_y, mask_weights, mask_head_predict,
                                                       segment_head_dims_->size(3), segment_head_dims_->size(2), mask_out_device,
                                                       mask_dim, mask_out_width, mask_out_height, stream_);

                                    checkCudaRuntime(cudaMemcpyAsync(mask_out_host->data, mask_out_device, bytes_of_mask_out,
                                                                     cudaMemcpyDeviceToHost, stream_));

                                    // resize to source image size

                                    CUDAKernel::resize_bilinear(mask_out_host->data, sizeof(uint8_t) * mask_out_width, mask_out_width, mask_out_height,
                                                                result_object_box.seg->data, int(pbox[2] - pbox[0]), int(pbox[3] - pbox[1]), stream_);
                                    checkCudaRuntime(cudaStreamSynchronize(stream_));
                                }
                            }
    ...
    std::shared_ptr<SegInfer> create_seg_infer(const std::string &engine_file, Task task, int gpuid, float confidence_threshold,
